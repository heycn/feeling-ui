<template>
  <div class="feel-date-range-picker" style="border: 1px solid red" ref="wrapper">
    <f-popover ref="popover" position="bottom" :container="popoverContainer" @open="onOpen">
      <f-input type="text" :value="formattedValue" @input="onInput" @change="onChange" ref="input" />
      <template slot="content">
        <div :class="c('pop')" @selectstart.prevent>
          <div :class="c('pickers')">
            <div :class="c('picker1')">
              <div class="feel-date-range-picker-nav">
                <span :class="c('prevYear', 'navItem')" @click="onClickPrevYear">
                  <f-icon name="leftleft" />
                </span>
                ...
                <span :class="c('nextYear', 'navItem')" @click="onClickNextYear">
                  <f-icon name="rightright" />
                </span>
              </div>
              <div class="feel-date-range-picker-panels">
                <div class="feel-date-range-picker-content">
                  <template v-if="mode === 'month'">
                    <div :class="c('selectMonth')">
                      <div :class="c('selects')">
                        <select @change="onSelectYear" :value="display.year">
                          <option v-for="year in years" :value="year" :key="year">{{ year }}</option></select
                        >年
                        <select @change="onSelectMonth" :value="display.month">
                          <option v-for="month in helper.range(0, 12)" :value="month" :key="month">
                            {{ month + 1 }}
                          </option></select
                        >月
                      </div>
          ...
          <div class="feel-date-range-picker-actions">
...
<script>
  ...
  export default {
    components: { FInput, FIcon, FPopover, FScroll, FButton },
    directives: { ClickOutside },
    name: 'FeelDateRangePicker',
    props: {
      firstDayOfWeek: {
        type: Number,
        default: 1
    ...
    methods: {
      c(...classNames) {
        return classNames.map(className => `feel-date-range-picker-${className}`)
      },
      onInput(value) {
        var regex = /^\d{4}-\d{2}-\d{2}$/f
