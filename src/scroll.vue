<template>
  <div
    ref="parent"
    class="feel-scroll-wrapper"
    @mouseenter="onMouseEnter"
    @mouseleave="onMouseLeave"
    @wheel="onWheel"
    @mousemove="onMouseMove"
  >
    <div ref="child" class="feel-scroll" :style="{ transform: `translateY(${this.contentY}px)` }">
      <slot></slot>
    </div>
    <div class="feel-scroll-track" v-show="scrollBarVisible">
      <div class="feel-scroll-bar" ref="bar" @mousedown="onMouseDownScrollBar" @selectstart="onSelectStartScrollBar">
        <div class="feel-scroll-bar-inner"></div>
      </div>
    </div>
  </div>
</template>
    ...
    methods: {
      listenToRemoteResources() {
        ...
        Array.from(tags).map(tag => {
          if (tag.hasAttribute('data-feel-listened')) {
            return
          }
          tag.setAttribute('data-feel-listened', 'yes')
      ...
      calculateContentYMax() {
        let { borderTopWidth, borderBottomWidth, paddingTop, paddingBottom } = window.getComputedStyle(
          this.$refs.parent
        )
