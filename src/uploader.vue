<template>
  <div class="feel-uploader">
    <div @click="onClickUpload">
      <slot></slot>
    </div>
    <ol class="feel-uploader-fileList">
      <li v-for="file in fileList" :key="file.name">
        <template v-if="file.status === 'uploading'">
          <g-icon name="loading" class="feel-uploader-spin"></g-icon>
        </template>
        <template v-else-if="file.type.indexOf('image') === 0">
          <img class="feel-uploader-image" :src="file.url" width="32" height="32" alt="" />
        </template>
        <template v-else>
          <div class="feel-uploader-defaultImage"></div>
        </template>

        <span class="feel-uploader-name" :class="{ [file.status]: file.status }">{{ file.name }}</span>
        <button class="feel-uploader-remove" @click="onRemoveFile(file)">x</button>
      </li>
    </ol>
